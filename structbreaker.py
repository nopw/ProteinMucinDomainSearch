...
def getdomainnew(sequence, domainlength, adjustmultiple, sequencelength, ptspercent, position):
    ...
    while( (lposition<len(sequence))and(sequence[lposition] != 'P') and (sequence[lposition] != 'T') and( sequence[lposition] != 'S') ):
    ...
    # domain is ds
        ...
        i = 1
        if (ldomain.domainseq.endswith('P') or ldomain.domainseq.endswith('T') or ldomain.domainseq.endswith('S')) and (lposition+ len(ldomain.domainseq))< len(sequence):
            
            nextword = sequence[lposition+ len(ldomain.domainseq)]
            while((nextword == 'P' or nextword == 'T' or nextword == 'S') and lposition+len(ldomain.domainseq)+1<len(sequence)):
            ...
            while not (ldomain.domainseq.endswith('P') or ldomain.domainseq.endswith('T') or ldomain.domainseq.endswith('S')):
...
def splitseqtodomain(sequence):    
    ...
    while not domainlist is None:
        ...
        domainlist = getdomainnew(sequencestr, defaultlength, adjustmultiple, sequencelength, ptspercent, domainobj.nextaddr)
    return oRet
...
def mergedomainbycondition(splitedsequencelist, minpercent, minlength=30, maxlength = 60):
    oRet = []
...
for db in dbs:
    ...
    for seq in SeqIO.parse(filehandle, 'fasta'):
        ...
        ltooltipcontent= '[sum(PTS):{0}, sequence length:{1},percent of PTS in sequence:{2}]'.format(loopsummaryinfo.ptscount,loopsummaryinfo.count,loopsummaryinfo.ptspercent)
        seqdiv= seqdiv+ r'<div><p style="color:red">No.:'+str(num)+' Sequence id:'+seq.id+r'</p></div>'+ ltooltipcontent+'<div></div><div><p class="nowrap">'
        ...
        if seq.id=='tr|A0A182G5J9|A0A182G5J9_AEDAL':
            c=1
        mergedomainlist = mergedomainbycondition(spliteddomainlist,0.70)
        i = 0
        ...
        for i in range(0, len(mergedomainlist)):
            ...
            if ldomain.domainkind == ldmkd.kindcs:
                ...
                tooltipcontent= '[sum(PTS):{0}, domain length:{1},percent of PTS in domain:{2},domainposition[{3}:{4}]]'.format(ssummaryinfo.ptscount,ssummaryinfo.count,ssummaryinfo.ptspercent, positiondomainstart,positiondomainend-1)
                seqdiv= seqdiv + r'<span data-tooltip="'+tooltipcontent+'" class="tooltip"><span id="dsbackground">'+ getmarkedds(ldomain) +r'</span></span>'
                dropseq = 0
        ...
        if dropseq==0:
            ...
            num = num +1
        finish=datetime.now() 
    outputfilecontent = r'<div><p>start:'+start.strftime('%Y-%m-%d %X')+'</p></div>'+outputfilecontent+r'<div><p>finish:'+finish.strftime('%Y-%m-%d %X')+'</p></div>'+r'</body></html>'
